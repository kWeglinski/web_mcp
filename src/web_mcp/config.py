...
class Config:
    ...
    def __init__(self):
        ...
        self.default_extractor: str = os.environ.get(
            ENV_DEFAULT_EXTRACTOR, "trafilatura"
        )
        self._validate_extractor()
        
        # Whether to include metadata
        self.include_metadata: bool = os.environ.get(
            ENV_INCLUDE_METADATA, "true"
        ).lower() in ("true", "1", "yes")
        
        # Whether to include links
        self.include_links: bool = os.environ.get(
            ENV_INCLUDE_LINKS, "false"
        ).lower() in ("true", "1", "yes")
        
        # Whether to include comments
        self.include_comments: bool = os.environ.get(
            ENV_INCLUDE_COMMENTS, "false"
        ).lower() in ("true", "1", "yes")
        ...
        self.truncation_strategy: str = os.environ.get(
            ENV_TRUNCATION_STRATEGY, "smart"
        )
        
        # SearXNG configuration
        self.searxng_url: Optional[str] = os.environ.get(
            ENV_SEARXNG_URL, None
        )
        ...
        self.cache_ttl: int = self._validate_int(
            os.environ.get(ENV_CACHE_TTL, "3600"), 1, 86400
        )
        
        # Playwright settings
        self.playwright_enabled: bool = os.environ.get(
            ENV_PLAYWRIGHT_ENABLED, "true"
        ).lower() in ("true", "1", "yes")
