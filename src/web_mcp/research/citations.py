...
def fix_citation_renumbering(text: str, sources: list[Source]) -> str:
    ...
    [s for s in sources if s.index in valid_indices]
    ...
    llm_numbers = sorted({int(m.group(1)) for m in matches if 1 <= int(m.group(1)) <= max_valid})
...
def normalize_citation_numbers(text: str) -> str:
    ...
    cited_numbers = sorted({int(m.group(1)) for m in matches})
