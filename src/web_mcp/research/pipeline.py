...
from web_mcp.fetcher import FetchError, fetch_url_with_fallback as fetch_url
from web_mcp.extractors.trafilatura import TrafilaturaExtractor
from web_mcp.searxng import search, SearXNGError
from web_mcp.llm.client import get_llm_client, LLMError
...
async def research(
    ...
    chunk_tuples = [
        (c.text, c.source_url, c.source_title, c.index)
        for c in all_chunks
    ]
    ...
    if rerank_enabled:
        try:
            relevant = await rerank_chunks(client, query, relevant, top_k=min(max_sources * 3, research_config.top_chunks))
        except Exception:
